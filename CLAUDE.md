...
| **Nối hai modal (A đóng → B mở)** | `command`/`commandFor` chỉ trỏ được MỘT modal. Giữ `command="--hide"` cho bước đóng, rồi mở B trong **`onAfterHide` của A** (`document.getElementById(id).showOverlay()`). 🔴 Mở B ngay trong `onClick` thì hai `<dialog>` tranh top layer và B bị đóng theo A. `onAfterHide` bắn ở MỌI lần đóng (Cancel · X · Esc) → **phải có cờ `useRef`**, không thì bấm Cancel cũng nhảy sang B. Verified 13 Aug 2026 ở luồng creator → model |
...
| **Radio group** | 🔴 `s-choice` **không có event riêng** — event nằm ở `s-choice-list` (`values: string[]`). Và `s-choice-list` chỉ nhận `Choice` làm con, nên **không bọc từng lựa chọn trong `s-box`** để làm thẻ: chèn box vào giữa là phá cấu trúc và mất luôn semantics radio |
| **`onInput`, KHÔNG `onChange`, cho `s-choice-list`** | 🔴 **Verified 13 Aug 2026 — `onChange` KHÔNG BAO GIỜ chạy.** `s-choice-list` bắn `change` với `composed: false` nên event dừng ở biên shadow DOM, mà React 18 gắn listener ở root container → handler không nhận được gì. Event `input` của cùng component đó là `composed: true` nên ra tới root và chạy đúng. **Bẫy im lặng tệ nhất từng gặp:** typecheck xanh, console sạch, radio/checkbox VẪN tự tick và `element.values` VẪN đúng (state nội bộ của component), chỉ state React đứng im — nên nhìn như đang hoạt động. Còn **2 chỗ đang sai**: `app.library.$id.tsx:277` (placements — không ai đọc `placements` nên chưa lộ) và `app.settings.tsx:762` (chọn "Only posts with a hashtag" **không hiện** field Hashtag). Sửa: đổi `onChange` → `onInput` |
