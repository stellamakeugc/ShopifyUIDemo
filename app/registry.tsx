...
export const MOCKUPS: Mockup[] = [
  ...
  },
  {
    ...
    open: [
      'Giá một video CHỐT là 150 credits (Stella 08 Aug 2026). Pricing 13 Aug 2026: Starter 300 ≈ ~2 video/tháng · Growth 2.000 ≈ ~13 · Scale 4.500 ≈ ~30. Ba con số credit vẫn cần Duong xác nhận.',
  ...
  },
  {
    ...
    description:
      'Tab **Creator video** → compose. GỘP Content Library Recreate + composer Talking Actors của platform làm một luồng. Bốn section: template (thumb trái, chữ phải) · Product (modal có search vì store có thể có hàng nghìn sản phẩm; description có nút AI "Summarise from product details") · Dialog (MỘT card, AI script writer là CHẾ ĐỘ bên trong đúng luồng platform, không phải ô nhập thứ hai; Add speech emotion chèn thẻ [excited] tại vị trí con trỏ) · Creator (**Optional**, mặc định TRỐNG — creator của template không nằm trong kho avatar, không đụng vào thì video giữ nguyên người có sẵn). **Giá động (13 Aug 2026):** giữ creator template = 150 credits; ĐỔI creator thì phải chọn model ở modal bước 2 — Seedance 2.0 (250) hoặc 2.5 (400) — và nút Generate ở header là nguồn duy nhất của con số cuối. Section Creator cảnh báo giá TRƯỚC khi bấm. Ba vùng credit: ≥400 cả hai · 250–399 khoá 2.5 kèm lý do · 150–249 khoá hẳn tính năng Creator nhưng video vẫn generate được với 150. ĐÃ BỎ state Free Forever — gói đó không có trong tập plan. Có gate disclaimer ba bước: banner CHẶN generate (nút Generate bị ẨN, không phải disable) → tick + Continue → banner "Thank you" tự tắt sau 3s có `CountdownRing` → Generate hiện ra. 16 state.',
    ...
    open: [
      ...
      '🛑 Giá Seedance 2.0 = 250 và 2.5 = 400 là TỰ ĐẶT. Hai cái tên là thật (Stella 13 Aug 2026), hai con số thì chưa — chọn 250/400 vì chúng tạo đủ ba vùng credit cần review quanh mức 150. Cần Duong chốt giá thật, và: 2.5 có bị chặn theo plan không (nếu có thì phải thêm state plan-gated cho model, khác hẳn state không-đủ-credit), đổi creator có làm ETA dài hơn 2–3 phút không.',
      '⏸️ Chọn model cấp TRANG (Quality/Mode) vẫn PARK từ 07 Aug 2026. Model quay lại 13 Aug 2026 nhưng chỉ nằm BÊN TRONG luồng đổi creator, không phải lựa chọn chất lượng độc lập ở đầu trang. Bảng `aiModels` cũ (Draft/Nova 2.0/Nova 2.0 HD) đã xoá khỏi sample.ts — nó là bảng chết, giữ cạnh `creatorModels` là mời dev đọc sai bảng.',
      'Allowance đã có từ pricing 13 Aug 2026 (Starter 300 · Growth 2.000 · Scale 4.500) nhưng CHƯA ai xác nhận ngoài Stella. Ở 150/video ra ~2 · ~13 · ~30 video/tháng. `TOTAL_CREDITS` của trang này derive từ `planCredits(\'scale\')`, không hardcode nữa.',
  ...
  },
  {
    ...
    open: [
      ...
      '🛑 Credit allowance theo pricing 13 Aug 2026: Starter 300 · Growth 2.000 · Scale 4.500 — chưa ai ngoài Stella xác nhận. Mọi state quota phụ thuộc tỉ lệ allowance / cỡ batch nên đổi số là phải xem lại các state đó.',
  ...
  },
  {
    ...
    description:
      'Vẽ lại 06 Aug 2026. **Nội dung pricing lấy từ Notion → Tactic 2 · Pricing proposal** (Stella chỉ đạo), sửa 5 chỗ mockup cũ bán thiếu — Triple Whale từ **Growth** chứ không phải Starter, Starter **unlimited widget**, credit **CÓ rollover** trên Growth/Scale, thêm bỏ-branding · sync review · migration. **Pricing mới 13 Aug 2026:** Starter 0 → **300** · Growth 500 → **2.000** · Scale 2.500 → **4.500**; thẻ và bảng in thêm **số video xấp xỉ** ở 150 credit/video (~2 · ~13 · ~30, làm tròn xuống); **BỎ dòng "Custom AI creator — your own brand actor"** vì tính năng không tồn tại; AI Studio giờ bắt đầu từ **Starter** nên gate "có credit" đổi thành `isGrowthUp()` (nếu không Starter tự được thêm 4 tính năng Growth). Action zone tối đa MỘT banner. Bảng **Plans comparison** 16 dòng × 3 nhóm, **thu gọn mặc định**, cột Most popular nổi bằng badge + in đậm (s-table-cell không nhận prop layout nên không tô nền cột được). FAQ accordion một-mở-một. 9 state.',
